...
module SemiStatic
  module SiteHelper
    ...
    def squeeze(entry)
      ...
      "<div id=\'squeeze-tease_#{entry.squeeze.id.to_s}\'><a class='squeeze' rel='nofollow' onclick='semiStaticAJAX(\"#{squeeze_path(s, :format => :js, :popup => true)}\")\; return false;' href='#{squeeze_path(s)}'>#{simple_format(s.teaser)}</a></div>".html_safe
    end
