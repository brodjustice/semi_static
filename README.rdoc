...
hence the name 'semi-static'. The HTML site should load very fast on mobile and desktop devices, and should get
a 100%, or there abouts, rating on Google pagespeed.

Some tricks used to get the fast performance:

1. No javascript loaded on home page, animations via HTML5
...
Assuming you want a clean applicaion to start with semi-static, first create your rails application:

1. Create your new rails application, for example using the postgres DB:

	# rails new myapp -d postgresql

2. With rails 3 your app will probably need a js runtime for the development environment, so uncomment that line in 'Gemfile':
...
1. Edit the Gemfile to include the semi-static gem/engine:

	gem 'semi_static', path: "../../engines/semi_static"

2. Run the bundler

	# bundle install

3. Run the semi-static install generator
